...
export default function ClothingSuggestion({ hourly }: Props) {
  ...
  return (
    <div className="bg-gray-50 rounded-xl px-4 py-4 flex items-center gap-2 text-gray-700 text-xl font-bold">
      <span>
        朝晩: <strong>{getClothing(morningEveningTemp)}</strong>{" "}
        <span className="text-gray-600 text-lg">({morningEveningTemp}°C)</span>
      </span>
      <span className="text-gray-300">|</span>
      <span>
        日中: <strong>{getClothing(daytimeTemp)}</strong>{" "}
        <span className="text-gray-600 text-lg">({daytimeTemp}°C)</span>
      </span>
    </div>
  );
}
